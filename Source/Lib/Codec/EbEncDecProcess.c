...
EbErrorType enc_dec_context_ctor(
    ...
    *context_dbl_ptr = context_ptr;
...
#if  ! FILT_PROC
    {
        ...
        //memset(context_ptr->trial_frame_rst->buffer_y,  context_ptr->trial_frame_rst->luma_size * 2, 0);
        //memset(context_ptr->trial_frame_rst->buffer_cb, context_ptr->trial_frame_rst->chroma_size * 2, 0);
        //memset(context_ptr->trial_frame_rst->buffer_cr, context_ptr->trial_frame_rst->chroma_size * 2, 0);
    ...
    EbFifo           *srmFifoPtr)
{
    ...
    case ENCDEC_TASKS_CONTINUE:
        ...
        if (rowSegmentIndex < segmentPtr->segment_row_count - 1 && bottomLeftSegmentIndex >= segmentPtr->rowArray[rowSegmentIndex + 1].startingSegIndex)
        {
...
void* enc_dec_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        {
            xLcuStartIndex = segments_ptr->xStartArray[segment_index];
            yLcuStartIndex = segments_ptr->yStartArray[segment_index];
            lcuStartIndex = yLcuStartIndex * picture_width_in_sb + xLcuStartIndex;
            lcuSegmentCount = segments_ptr->validLcuCountArray[segment_index];
            ...
            segmentBandSize = (segments_ptr->lcuBandCount * (segmentBandIndex + 1) + segments_ptr->segment_band_count - 1) / segments_ptr->segment_band_count;
        ...
        if (lastLcuFlag) {
            ...
            if (sequence_control_set_ptr->enable_restoration) {
                ...
                memcpy(&picture_control_set_ptr->parent_pcs_ptr->av1x->sgrproj_restore_cost, &context_ptr->md_rate_estimation_ptr->sgrproj_restore_fac_bits, 2 * sizeof(int32_t));
                memcpy(&picture_control_set_ptr->parent_pcs_ptr->av1x->switchable_restore_cost, &context_ptr->md_rate_estimation_ptr->switchable_restore_fac_bits, 3 * sizeof(int32_t));
                memcpy(&picture_control_set_ptr->parent_pcs_ptr->av1x->wiener_restore_cost, &context_ptr->md_rate_estimation_ptr->wiener_restore_fac_bits, 2 * sizeof(int32_t));
                picture_control_set_ptr->parent_pcs_ptr->av1x->rdmult = context_ptr->full_lambda;
            ...
            if (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) {
                ...
                pictureDemuxResultsPtr = (PictureDemuxResults*)pictureDemuxResultsWrapperPtr->object_ptr;
