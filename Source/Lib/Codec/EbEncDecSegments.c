    ...
    uint32_t            pic_height_lcu)
{
    ...
    EB_MEMSET(segments_ptr->valid_lcu_count_array, 0, sizeof(uint16_t) * segments_ptr->segment_total_count);
    ...
    for (y = 0; y < pic_height_lcu; ++y) {
        for (x = 0; x < pic_width_lcu; ++x) {
            bandIndex = BAND_INDEX(x, y, segments_ptr->segment_band_count, segments_ptr->lcu_band_count);
    ...
    for (row_index = 0; row_index < segments_ptr->segment_row_count; ++row_index) {
        ...
        bandIndex = BAND_INDEX(0, y, segments_ptr->segment_band_count, segments_ptr->lcu_band_count);

        segments_ptr->row_array[row_index].starting_seg_index = (uint16_t)SEGMENT_INDEX(row_index, bandIndex, segments_ptr->segment_band_count);
        bandIndex = BAND_INDEX(pic_width_lcu - 1, yLast, segments_ptr->segment_band_count, segments_ptr->lcu_band_count);
        segments_ptr->row_array[row_index].ending_seg_index = (uint16_t)SEGMENT_INDEX(row_index, bandIndex, segments_ptr->segment_band_count);
