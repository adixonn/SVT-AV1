...
EbErrorType mode_decision_context_ctor(
    ...
    *context_dbl_ptr = context_ptr;
    ...
    for (codedLeafIndex = 0; codedLeafIndex < BLOCK_MAX_COUNT_SB_128; ++codedLeafIndex) {
        ...
        UNUSED(blk_geom);
        EB_MALLOC(MacroBlockId*, context_ptr->md_cu_arr_nsq[codedLeafIndex].av1xd, sizeof(MacroBlockId), EB_N_PTR);
...

{
    ...
    if (bit_depth == 8) {
        ...
        *full_lambda = av1lambda_mode_decision8_bit_sse[qp_index];
        *fast_lambda = av1lambda_mode_decision8_bit_sad[qp_index];
