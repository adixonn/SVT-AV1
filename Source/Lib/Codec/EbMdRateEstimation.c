    ...
    FrameContext              *fc)
{
    ...
    for (i = 0; i < SWITCHABLE_FILTER_CONTEXTS; ++i)
        av1_get_syntax_rate_from_cdf(md_rate_estimation_array->switchable_interp_fac_bits[i], fc->switchable_interp_cdf[i], NULL);
    ...
    for (i = 0; i < PALATTE_BSIZE_CTXS; ++i) {
        ...
        for (j = 0; j < PALETTE_Y_MODE_CONTEXTS; ++j) {
            av1_get_syntax_rate_from_cdf(md_rate_estimation_array->palette_y_mode_fac_bits[i][j], fc->palette_y_mode_cdf[i][j], NULL);
        }
    }
    ...
    for (i = 0; i < PALETTE_SIZES; ++i) {
        for (j = 0; j < PALETTE_COLOR_INDEX_CONTEXTS; ++j) {
            av1_get_syntax_rate_from_cdf(md_rate_estimation_array->palette_y_color_fac_bitss[i][j], fc->palette_y_color_index_cdf[i][j], NULL);
